...
"""
This file implements the SIPP_cleaning function in "aSIPP.py" file step by
step. Codes in this script file consistute the foundation of the SIPP_cleaning 
function.
...
To transform it into a function, we need to make the following changes:
    Take codes_path as an input argument. 
    Take panel year as an input argument.
    Read raw dta files for each panel. 
    Write the DataFrame to a dta file storing in tempdata folder.

Input:
    dta files in 1996 panel stored in "rawdata" folder.
Output:
    tempdata/spell1996.dta
...
print(val_labs["occ1990dd"][43])  # test value labels

# -?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?
# -? s-0-3. Functions to recode SIPP occupation codes
# -? (stored in codes/util/occ.py)
# -?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?
from util.occ import RecodeOcc_Panel96And01

help(RecodeOcc_Panel96And01)  # test functions

# -?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?
# -? s-0-4. Functions for DataFrame manipulation
...
sippw1 = pd.read_stata(rawdata("sipp96w1.dta"), convert_categoricals=False)
sippw2 = pd.read_stata(rawdata("sipp96w2.dta"), convert_categoricals=False)
sippw3 = pd.read_stata(rawdata("sipp96w3.dta"), convert_categoricals=False)
sippw4 = pd.read_stata(rawdata("sipp96w4.dta"), convert_categoricals=False)
sippw5 = pd.read_stata(rawdata("sipp96w5.dta"), convert_categoricals=False)
sippw6 = pd.read_stata(rawdata("sipp96w6.dta"), convert_categoricals=False)
sippw7 = pd.read_stata(rawdata("sipp96w7.dta"), convert_categoricals=False)
sippw8 = pd.read_stata(rawdata("sipp96w8.dta"), convert_categoricals=False)
sippw9 = pd.read_stata(rawdata("sipp96w9.dta"), convert_categoricals=False)
sippw10 = pd.read_stata(rawdata("sipp96w10.dta"), convert_categoricals=False)
sippw11 = pd.read_stata(rawdata("sipp96w11.dta"), convert_categoricals=False)
sippw12 = pd.read_stata(rawdata("sipp96w12.dta"), convert_categoricals=False)
...
vars_all = vars_id + vars_demogr + vars_emp + vars_occ + vars_earn + vars_wgt
...
temp.loc[((temp["eclwrk1"].isin([3, 4, 5])) & (temp["empl"] == 1)), "gov1"] = 1
temp.loc[((temp["eclwrk2"].isin([3, 4, 5])) & (temp["empl"] == 1)), "gov2"] = 1
...
temp["disc_spell"] = np.where(
    (
        ((temp["next_indid"] == temp["indid"]) & (temp["next_indid"].notna()))
        & (
...
temp.loc[temp["disc_spell"] == 1, "cont_spell_no"] = temp["cont_spell_no"] - 1
...
# -? if the individual is not employed, then he can be defined as unemployed
# -?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?#-?
...
temp["nxt_empl"] = temp.groupby(["indid", "cont_spell_no"])["empl"].shift(-1)
temp["lst_empl"] = temp.groupby(["indid", "cont_spell_no"])["empl"].shift(1)
...
temp.loc[((temp["lst_empl"] == 1) & (temp["empl"] == 0)), "start_of_ubar"] = 1
...
temp.loc[((temp["nxt_empl"] == 1) & (temp["empl"] == 0)), "end_of_ubar"] = 1
...
temp["spellmax_unempl"] = temp.groupby("ubar_spell_no")["unempl"].transform(
    "max"
)

temp["ustar_spell_no"] = temp["ubar_spell_no"]
temp.loc[(temp["spellmax_unempl"] == 0), "ustar_spell_no"] = np.nan
# && 7,698 E(USTAR)E spells left
...
temp["spellsum_unempl"] = temp.groupby("ubar_spell_no")["unempl"].transform(
    "sum"
)

temp["u_spell_no"] = temp["ubar_spell_no"]
temp.loc[(temp["spellsum_unempl"] != temp["len_ubar_spell"]), "u_spell_no"] = (
    np.nan
)
# && 5,069 E(U)E spells left
...
temp.loc[temp["empl"] == 1, "firmid"].isna().sum()
# && For 101 observations, they are employed but we fail to assign a firm number.
...
temp.loc[(temp["empl"] == 1), "occ_raw"].isna().sum()  # 15391
...
end_of_ubar_info["next_empl_month"] = end_of_ubar_info["ym"] + pd.DateOffset(
    months=1
)
...
print(f"Number of num individuals in all E(UBAR)E spells: {num_indid_ubar}")

# -? number of individuals in E(USTR)E spells
num_indid_ustar = temp.loc[temp["ustar_spell_no"].notna(), "indid"].nunique()
print(f"Number of num individuals in all E(UBAR)E spells: {num_indid_ustar}")
...
num_ubar_occ = temp.loc[
    ((temp["source_occ_raw"].notna()) & (temp["destination_occ_raw"].notna())),
    "ubar_spell_no",
].nunique()
print(f"Number of E(UBAR)E spells with identified occ info: {num_ubar_occ}")
...
print(f"Number of E(USTAR)E spells with identified occ info: {num_ustar_occ}")
